 ...
 */
import { onDOM, offDOM, hasClass, findParent, toArrayIfNot, getPosition, getBoundingClientRect, backupAttr, addClass, restoreAttr, getViewportPosition, elementsFromPoint, isDescendantOf } from 'helper-js';
...
function index (listenerElement) {
  ...
  var onMousemoveOrTouchMove = function onMousemoveOrTouchMove(e, mouse) {
    ...
    if (store.movedCount === 0) {
      ...
      var initialPosition = getPosition(movedOrClonedElement); // attach elements and initialPosition to store
      // 附加元素和初始位置到store
...
function fixScrollBox(boxElement) {
  ...
  var onMove = function onMove(e, mouse) {
    ...
    if (boxElement.scrollHeight > boxElement.offsetHeight || boxElement.scrollWidth > boxElement.offsetWidth) {
      var boxPosition = getViewportPosition(boxElement);
      ...
      var beforeEveryFrame = function beforeEveryFrame(count) {
        ...
        var _iterator2 = _createForOfIteratorHelper(elementsFromPoint(currentMousePosition.clientX, currentMousePosition.clientY)),
            _step2;
        ...
        if (firstElement !== foundBoxElement && !isDescendantOf(firstElement, foundBoxElement)) {
...
export { allListeningElementsOfFixScrollBox, defaultOptions, defaultOptionsForFixScrollBox, fixScrollBox, initialStore };
